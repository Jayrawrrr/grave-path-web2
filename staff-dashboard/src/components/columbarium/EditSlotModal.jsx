...
const EditSlotModal = ({ slot, onClose, onSave }) => {
  ...
  useEffect(() => {
    if (slot) {
      setFormData({
        building: slot.building || 'Main Building',
        floor: slot.floor || 1,
        section: slot.section || 'A',
        row: slot.row || 1,
        column: slot.column || 1,
        size: slot.size || 'single',
        dimensions: slot.dimensions || { width: 30, height: 30, depth: 30 },
        price: slot.price || 50000,
        status: slot.status || 'available',
        position: slot.position || { x: 0, y: 0, z: 0 }
      });
    }
  }, [slot]);
